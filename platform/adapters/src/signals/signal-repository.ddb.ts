/**
 * SignalRepository - DynamoDB Implementation
 *
 ...
 * SignalRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing Signal repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { SignalRepository } from "@chattermark/services/signals";
...
/**
 * SignalRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: SIGNAL
 * Entity Token: SIGNAL
 * SK Prefix: SIGNAL
...
export class SignalRepositoryDdb implements SignalRepository {
  ...
  private readonly ENTITY_TYPE = "SIGNAL";
  private readonly ENTITY_TOKEN = "SIGNAL";
  private readonly SK_PREFIX = "SIGNAL";
  ...
  private readonly DOMAIN = "SIGNALS";
  private readonly ENTITY_TYPE_PK = "SIGNALS";
  private generateCorrelationId(): string {
    const domainPrefix = "sig";
  ...
  async getSignal(input: Parameters<SignalRepository['getSignal']>[0]): Promise<Awaited<ReturnType<SignalRepository['getSignal']>>> {
    ...
    if (!(inputExt.id ?? inputExt.signalId)) {
      throw new Error("Missing required parameter: id");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.signalId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.signalId)}#`;
    const result = await this.dynamoClient.query({
      TableName: this.TABLE_NAME,
      KeyConditionExpression: "PK = :pk AND begins_with(SK, :skPrefix)",
      ExpressionAttributeValues: { ":pk": pk, ":skPrefix": skPrefix },
    ...
    const item = result.Items?.[0];
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.signalId)} not found`);
    }
    return {
      data: sanitizeItem(item as Record<string, unknown>) as unknown as any,
      meta: {
