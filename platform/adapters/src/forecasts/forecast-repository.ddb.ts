/**
 * ForecastRepository - DynamoDB Implementation
 *
...
/**
 * ForecastRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing Forecast repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { ForecastRepository } from "@chattermark/services/forecasts";
...
/**
 * ForecastRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: FORECAST
 * Entity Token: FORECAST
 * SK Prefix: FORECAST
 */
export class ForecastRepositoryDdb implements ForecastRepository {
  private readonly TABLE_NAME: string;
  // Metadata from x-dynamodb annotation - all values are dynamic
  private readonly ENTITY_TYPE = "FORECAST";
  private readonly ENTITY_TOKEN = "FORECAST";
  private readonly SK_PREFIX = "FORECAST";
  ...
  private readonly DOMAIN = "FORECASTS";
  private readonly ENTITY_TYPE_PK = "FORECASTS";
  private generateCorrelationId(): string {
    const domainPrefix = "fct";
  ...
  async getForecast(input: Parameters<ForecastRepository['getForecast']>[0]): Promise<Awaited<ReturnType<ForecastRepository['getForecast']>>> {
    ...
    if (!(inputExt.vulnerabilityId ?? inputExt.id ?? inputExt.vulnerabilityId)) {
      throw new Error("Missing required parameter: vulnerabilityId");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.vulnerabilityId ?? inputExt.id ?? inputExt.vulnerabilityId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.vulnerabilityId ?? inputExt.id ?? inputExt.vulnerabilityId)}#`;
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.vulnerabilityId ?? inputExt.id ?? inputExt.vulnerabilityId)} not found`);
    }
    return {
